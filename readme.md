...
Структура положения условно разделена на два уровня: `engine` и `application`. На первом уровне выполняется установка движка: создаются рендеры, камеры, определяется ключевые компоненты системы. На уровне приложение реализуются логика и сущности предметной области: магазины, hover-эфект, обработка кликов и т.д.
